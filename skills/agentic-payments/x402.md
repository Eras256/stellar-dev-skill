...
// is set. When it isn't, a request to one of the keys above still gets a
// 503 — never a free 200 — while a request to any other path still falls
// through unpriced, same as always. Same rule as mpp.md's charge/channel
// middleware: a paid production path degrades loudly, never silently.
let x402Middleware;
if (PAY_TO) {
  ...
  x402Middleware = (req, res, next) => {
    if (`${req.method} ${req.path}` in PAID_ROUTES) {
...
`PAY_TO` is set or not. This is what's running behind Nirium's own
